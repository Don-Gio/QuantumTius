...
int OnInit()
  {
   ...
   Print("Kaku - XAU/USD Signal Bot Initialisé !");
...
void OnTick()
  {
   ...
   if (MovingAverageCurrent > MovingAveragePrev && RSIValue < OversoldLevel)
     {
      BuySignal = true;
      SellSignal = false;
      Print("💥 Signal d'Achat détecté sur XAU/USD !");
     }
   // Vérification des conditions de vente
   else if (MovingAverageCurrent < MovingAveragePrev && RSIValue > OverboughtLevel)
     {
      BuySignal = false;
      SellSignal = true;
      Print("💥 Signal de Vente détecté sur XAU/USD !");
     }
   ...
   if (BuySignal)
     {
      if (OrderSend(Symbol(), OP_BUY, 0.1, Ask, 2, 0, 0, "Kaku - Buy Signal", 0, 0, clrGreen) < 0)
        {
   ...
   if (SellSignal)
     {
      if (OrderSend(Symbol(), OP_SELL, 0.1, Bid, 2, 0, 0, "Kaku - Sell Signal", 0, 0, clrRed) < 0)
        {
...
void OnDeinit(const int reason)
  {
   Print("Kaku - XAU/USD Signal Bot arrêté. À la prochaine !");
  }
